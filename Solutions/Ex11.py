...
def max_product_in_grid(grid):
    ...
    for r in range(rows):
        for c in range(cols):
            # Finding the product of the number and the 3 adjacent numbers on the right
            if c + 3 < cols:
                prod = grid[r][c] * grid[r][c+1] * grid[r][c+2] * grid[r][c+3]
                if prod > max_product:
                    max_product = prod
            # Finding the product of the number and the 3 adjacent numbers vertically under the number
            if r + 3 < rows:
                prod = grid[r][c] * grid[r+1][c] * grid[r+2][c] * grid[r+3][c]
                if prod > max_product:
                    max_product = prod
            # Finding the product of the number and the 3 adjacent numbers on the right diagonally (down)
            if r + 3 < rows and c + 3 < cols:
                prod = grid[r][c] * grid[r+1][c+1] * grid[r+2][c+2] * grid[r+3][c+3]
                if prod > max_product:
                    max_product = prod
            # Finding the product of the number and the 3 adjacent numbers on the right diagonally (up)
